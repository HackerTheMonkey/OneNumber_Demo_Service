...
public abstract class ProxySbb extends OCSipSbb
{
    ...
    protected ProxyRouter createProxyRouter(Context myEnv) throws NamingException
    {
        ...
        ProxyConfig config = new ProxyConfig()
        {
            ...
            public OCSleeSipProvider getSipProvider()
            {
                return ProxySbb.this.getSleeSipProvider();
            }

            public AddressFactory getSipAddressFactory()
            {
                return ProxySbb.this.getSipAddressFactory();
            }

            public OCHeaderFactory getSipHeaderFactory()
            {
                return ProxySbb.this.getOCHeaderFactory();
            }

            public MessageFactory getSipMessageFactory()
            {
                return ProxySbb.this.getSipMessageFactory();
            }

            public String[] getProxyDomains()
            {
                return domains;
            }

            public boolean isRecordRouteEnabled()
            {
    ...
    public void proxyRequestStateless(final Request request)
    {
        ...
        try
        {
            ...
            for (int i = 0; i < requestsToSend.length; i++)
            {
                ...
                {
                    fine("forwarding: " + request.getMethod() + " to target: " + requestsToSend[i].getRequestURI());
                }
                if (isTraceable(TraceLevel.FINEST))
                {
                    finest("forwarding request statelessly:\n" + requestsToSend[i]);
                }
                try
                {
    ...
    protected void doProxyRequest(final Request request, URI[] targets)
    {
        ...
        if (getProxyState() != PROXY_INIT)
        {
            throw new IllegalStateException("already proxying request");
        }
        ...
        try
        {
            ...
            for (int i = 0; i < requestsToSend.length; i++)
            {
                ...
                if (getInvite() && aci.getResponseStatusCode() == 0)
                {
                    setTimerC(aci);
                }
            }
            ...
            final ClientTransaction ct)
    {
        ...
        if (stateless)
        {
            ...
            else
            {
                if (isTraceable(TraceLevel.FINEST))
                {
                    finest("not a 2xx response to INVITE, dropping");
                }
                return;
            }
        ...
        if (getInvite() && !stateless)
        {
            updateTimerC(responseContextACI, newResponse);
        }
        ...
        if (listener != null)
        {
            listener.receivedResponse(ct, newResponse);
        }
        else
        {
            receivedResponse(ct, newResponse);
        }
    ...
    protected void processResponseContexts()
    {
        ...
        if (acis.length == 0)
        {
            return; // nothing to do, no "response context"
        }
        if (getProxyState() == PROXY_SENT_FINAL_RESPONSE)
        {
        ...
        else
        {
            ...
            for (int i = 0; i < acis.length; i++)
            {
                ...
                if (activity instanceof ClientTransaction || activity instanceof NullActivity)
                {
                    ...
                    // The proxy only creates null ACIs with response status code set - therefore if
                    // response status code is still zero then this isn't our null activity, so ignore it.
                    if (statusCode == 0 && activity instanceof NullActivity)
                    {
                        continue;
                    }
    ...
    protected Response generateErrorResponse(ProxyResponseContextACI proxyACI)
    {
        ...
        if (st == null)
        {
            throw new RuntimeException("no server transaction attached, cannot generate response");
        }
    ...
    protected void handleOptions(RequestEvent optionsEvent)
    {
        if (isTraceable(TraceLevel.FINE))
        {
            fine("received OPTIONS directed to this proxy, generate OK response");
        }
    ...
    protected void sendResponse(Response response)
    {
        ...
        if (listener != null)
        {
            listener.forwardResponse(response);
        }
        else
        {
    ...
    protected ProxyResponseContextACI forwardRequest(Request request)
    {
        ...
        catch (Throwable e)
        {
            ...
            if (proxyACI != null)
            {
                proxyACI.detach(getSbbLocalObject());
            }
