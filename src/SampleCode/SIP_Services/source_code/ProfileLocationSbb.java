...
public abstract class ProfileLocationSbb extends BaseSbb
{
    ...
    public void onTimerEvent(TimerEvent event, RegistrationExpiryACI regExpiryACI)
    {
        ...
        {
            finer("received expiration timer event for contact: " + publicAddress + "/" + contactAddress);
        }
    ...
    {
        // no-op - profile has already been updated in current transaction
    }

    public void removeRegistration(Registration reg)
    {
        removeProfile("removeRegistration", ((RegistrationImpl) reg).getProfile());
    }
    ...
    private RegistrationProfileLocal lookupRegistration(String context, ProfileTable table, String addressOfRecord, boolean create)
    {
        ...
        {
            finest(context + ": lookup profile for address-of-record \"" + addressOfRecord + "\"");
        }
        ...
        if (profile == null)
        {
            if (isTraceable(TraceLevel.FINEST))
            {
                finest(context + ": no profile matching address-of-record \"" + addressOfRecord + (create ? "\", create" : "\""));
            }
            if (create)
            {
                try
                {
                    ...
                    if (isTraceable(TraceLevel.FINEST))
                    {
                        finest(context + ": created profile " + profile.getProfileTableName() + "/" + profile.getProfileName());
                    }
                }
        ...
        else
        {
            if (isTraceable(TraceLevel.FINEST))
            {
                finest(context + ": found profile " + profile.getProfileTableName() + "/" + profile.getProfileName());
            }
            return profile;
        }
    }

    private void removeProfile(String context, RegistrationProfileLocal profile)
    {
        if (isTraceable(TraceLevel.FINEST))
        {
            finest(context + ": removing profile " + profile.getProfileTableName() + "/" + profile.getProfileName());
        }
        profile.remove();
    }
    ...
    private void startExpiryTimer(String addressOfRecord, ContactImpl contact)
    {
        ...
        if (isTraceable(TraceLevel.FINEST))
        {
            finest("set registration expiry timer for " + contact.getContactURI()
    ...
    private class RegistrationImpl implements Registration
    {
        ...
        RegistrationProfileLocal getProfile()
        {
            return profile;
        }
        ...
        {
            return profile.getAddressOfRecord();
        }
        ...
        {
            return profile.getContact(contactURI);
        }

        public Contact getContact(FlowID flowID)
        {
            return profile.getContact(flowID);
        }
        ...
        public Contact removeContact(String contactURI)
        {
            ...
            if (contact != null)
            {
                if (isTraceable(TraceLevel.FINEST))
                {
                    finest("removeContact: " + contactURI);
                }
                // stop timer
                TimerID id = contact.getExpiryTimer();
                if (id != null)
                {
                    getTimerFacility().cancelTimer(id);
                }
            }
            return contact;
        }
        ...
        public Contact removeContact(FlowID flowID)
        {
            ...
            if (contact != null)
            {
                if (isTraceable(TraceLevel.FINEST))
                {
                    finest("removeContact: flow=" + flowID);
                }
                // stop timer
                TimerID id = contact.getExpiryTimer();
                if (id != null)
