...
public abstract class EventStateCompositorSbb extends PresenceAwareSbb
{
    ...
    {
        return "EventStateCompositorSbb";
    }
    ...
    public void onPresentityStateChangeEvent(PresentityStateChangeEvent event, ActivityContextInterface aci)
    {
        ...
        {
            finest("PresentityStateChangeEvent event received by EventStateCompositorSbb: " + event.toString());
        }
    ...
    public void onExpiryTimer(TimerEvent timerEvent, PresenceACI aci)
    {
        ...
        if (isTraceable(TraceLevel.FINEST))
        {
            finest("onExpiryTimer event received by EventStateCompositorSbb");
        }
        aci.detach(getSbbLocalObject());
        if (aci.getTimerCount() > 0)
        {
            aci.setTimerCount(aci.getTimerCount() - 1);
        }
        ...
        if (presenceInfo != null)
        {
            ...
            if (isTraceable(TraceLevel.FINEST))
            {
                finest("onExpiryTimer: initial presence state of presentity is: " + originalState.toString());
            }
            for (int i = 0; i < presenceInfo.length; i++)
            {
                ...
                if (isTraceable(TraceLevel.FINEST))
                {
                    finest("onExpiryTimer: current time is: " + System.currentTimeMillis() + "; expiry time of presence record is: " + expiryTime);
                }
            ...
            if (isTraceable(TraceLevel.FINEST))
            {
                finest("onExpiryTimer: new presence state of presentity is: " + determineCurrentPresentityState(presenceInfo).toString());
            }
            if (!originalState.equals(determineCurrentPresentityState(presenceInfo)))
            {
                if (isTraceable(TraceLevel.FINEST))
                {
                    finest("onExpiryTimer: Presence state has changed, attempting to alert NotifySbbs");
                }
    ...
    private void updatePresenceInformation(PresentityStateChangeEvent event)
    {
        ...
        if (isTraceable(TraceLevel.FINEST))
        {
            finest("updatePresenceInformation: Updating presence information for " + event.getSipAddressOfRecord());
        }
        ...
        if (isTraceable(TraceLevel.FINEST))
        {
            finest("updatePresenceInformation: oldState = " + oldState.toString());
        }
        PresentityState newState = event.getPresenceState() == null ? oldState : event.getPresenceState();
        if (isTraceable(TraceLevel.FINEST))
        {
            finest("updatePresenceInformation: initial value of newState = " + newState.toString());
        }
        ...
        if (presenceInfo == null)
        {
            ...
            if (isTraceable(TraceLevel.FINEST))
            {
                finest("updatePresenceInformation: No presence info found for " + event.getSipAddressOfRecord() + ", creating new presence record");
            }
        ...
        else
        {
            ...
            if (event.getSipETag() != null && isTraceable(TraceLevel.FINEST))
            {
                finest("updatePresenceInformation: received presence information derived from a PUBLISH request");
            }
            for (i = 0; i < presenceInfo.length; i++)
            {
                    ...
                    if (event.getSipIfMatch() != null)
                    {
                        ...
                        if (presenceInfo[i][PresenceACI.ETAG] != null && event.getSipIfMatch().equals(presenceInfo[i][PresenceACI.ETAG]))
                        {
                            if (event.getExpiryTime() <= System.currentTimeMillis())
                            {
                                ...
                                if (isTraceable(TraceLevel.FINEST))
                                {
                                    finest("updatePresenceInformation: expiryTime of event is: " + event.getExpiryTime() + ", current time is: " + System.currentTimeMillis() + ", removing existing record");
                                }
                            ...
                            else
                            {
                                ...
                                if (isTraceable(TraceLevel.FINEST))
                                {
                                    finest("updatePresenceInformation: expiryTime of event is: " + event.getExpiryTime() + ", current time is: " + System.currentTimeMillis() + ", updating existing record");
                                }
                ...
                else
                {
                    ...
                    if (isTraceable(TraceLevel.FINEST))
                    {
                        finest("updatePresenceInformation: received presence information derived from a REGISTER request...");
                    }
            ...
            if (i == length && isTraceable(TraceLevel.FINEST))
            {
                finest("updatePresenceInformation: no matching records found, adding a new record...");
            }
            if (i == length)
            {
                presenceInfo = addNewPresenceRecord(presenceInfo, event);
            }
        }

        // If required, set a timer to fire when the requested expiry time is exhausted
        if (event.getExpiryTime() > 0 && event.getExpiryTime() > System.currentTimeMillis())
        {
            setExpiryTimer(presenceAci, event.getExpiryTime());
        }
        ...
        if (getSubscriptionACI(event.getSipAddressOfRecord()) != null)
        {
            if (!oldState.equals(newState))
            {
                if (isTraceable(TraceLevel.FINEST))
                {
                    finest("updatePresenceInformation: old and new states don't match, sending notification of state change...");
                }
            ...
            else
            {
                if (isTraceable(TraceLevel.FINEST))
                {
                    finest("updatePresenceInformation: old and new states match, no notifications necessary");
                }
            }
        }
    }
    ...
    private PresentityState determinePresentityState(String[][] presenceInformation, boolean onlyActiveStates)
    {
        ...
        if (presenceInformation == null)
        {
            if (isTraceable(TraceLevel.FINEST))
            {
                finest("No presence information stored for this presentity at present. Returning default presence state (closed/null)");
            }
        ...
        for (int i = 0; i < presenceInformation.length; i++)
        {
            ...
            else
            {
                ...
                if (basic.equalsIgnoreCase(PresentityState.CLOSED) && note == null)
                {
                    return new PresentityState(basic, null);
                }
            }
        }
        ...
        if (basic == null)
        {
            basic = PresentityState.CLOSED;
        }
        if (isTraceable(TraceLevel.FINEST))
        {
            finest("determinePresentityState: Determined presence state of: " + basic + "/" + note);
        }
    ...
    private String[][] addNewPresenceRecord(String[][] presenceInfo, PresentityStateChangeEvent event)
    {
        ...
        if (isTraceable(TraceLevel.FINEST))
        {
            finest("Adding new record to presence info for: " + event.getSipAddressOfRecord());
        }
    ...
    private String[][] removeNullRecords(String[][] presenceInfo)
    {
        ...
        for (int i = 0; i < presenceInfo.length; i++)
        {
            if (presenceInfo[i] != null)
            {
                tempList.add(presenceInfo[i]);
            }
        }
    ...
    private void setExpiryTimer(PresenceACI presenceAci, long expiryTime)
    {
        ...
        if (isTraceable(TraceLevel.FINEST))
        {
            finest("setExpiryTimer: Starting timer, setting to fire at " + expiryTime);
        }
    }
    ...
    private void unbindAci(PresenceACI aci)
    {
        if (isTraceable(TraceLevel.FINEST))
        {
            finest("unbindAci: unbinding aci from AC naming, using uri: " + aci.getSipUri());
        }
        if (getPresenceACI(aci.getSipUri()) != null)
        {
            removePresenceACI(aci.getSipUri());
        }
    }
    ...
    private void unbindAciFromACNaming(PresenceACI aci)
    {
        ...
        if (presenceInfo == null || (presenceInfo.length == 1 && determineCurrentPresentityState(presenceInfo).equals((new PresentityState(PresentityState.CLOSED, null)))))
        {
            ...
            if (aci.getTimerCount() == 0)
            {
                if (isTraceable(TraceLevel.FINEST))
                {
                    finest("unbindAciFromACNaming: attempting to unbind aci bound with uri of: " + aci.getSipUri());
                }
                unbindAci(aci);
            }
        }
