...
public abstract class RegistrarSbb extends PresenceAwareSbb
{
    ...
    public void onRegisterEvent(RequestEvent event, ActivityContextInterface aci)
    {
        ...
        try
        {
            ...
            if (request.getHeader(ContactHeader.NAME) == null)
            {
                ...
                {
                    fine("received registration query for: " + sipAddressOfRecord);
                }
            ...
            for (Iterator it = request.getHeaders(ContactHeader.NAME); it.hasNext();)
            {
                ...
                if (contact.getAddress().isWildcard())
                {
                    removeAll = true;
                }
            ...
            if (removeAll)
            {
                if (isTraceable(TraceLevel.FINE))
                {
                    fine("received remove all request for: " + sipAddressOfRecord);
                }
                ...
                if ((expiresHeader == null) || (expiresHeader.getExpires() != 0) || (newContacts.size() > 1))
                {
                    if (isTraceable(TraceLevel.FINE))
                    {
                        fine("remove all request malformed");
                    }
            ...
            if (isTraceable(TraceLevel.FINE))
            {
                fine("received registration change request for: " + sipAddressOfRecord);
            }
            ...
            while (it.hasNext())
            {
                ...
                if (requestedExpires == -1)
                {
                    return; // error response has been sent
                }
                if (isTraceable(TraceLevel.FINER))
                {
                    fine("expire time for contact [" + contact.getAddress() + "] is " + requestedExpires + "s");
                }
                ...
                if (existingEntry == null)
                {
                    ...
                    if (requestedExpires != 0)
                    {
                        if (isTraceable(TraceLevel.FINE))
                        {
                            fine("adding registration: " + sipAddressOfRecord + " -> " + contactAddress
                ...
                else
                {
                    ...
                    if (requestedExpires == 0)
                    {
                        if (flowID == null)
                        {
                            if (isTraceable(TraceLevel.FINE))
                            {
                                fine("removing registration: " + sipAddressOfRecord + " -> " + contactAddress);
                            }
                        ...
                        else
                        {
                            if (isTraceable(TraceLevel.FINE))
                            {
                                fine("removing registration: " + sipAddressOfRecord + " -> " + contactAddress + ", flow=" + flowID);
                            }
                    ...
                    else
                    {
                        if (isTraceable(TraceLevel.FINE))
                        {
                            fine("updating registration: " + sipAddressOfRecord + " -> " + contactAddress
                                    + (flowID != null ? ", flow=" + flowID : ""));
                        }
            ...
            if (isTraceable(TraceLevel.FINE))
            {
                fine("Attempting to create PresentityState with values: " + (reg.getContacts().isEmpty() ? PresentityState.CLOSED : PresentityState.OPEN) + "/null");
            }
    ...
    private void processRemoveAll(ServerTransaction st, LocationService ls, String uri, String callId, int cseq)
    {
        ...
        if (reg != null)
        {
            ...
            {
                Registration.Contact contact = (Registration.Contact) it.next();
                if (!callId.equals(contact.getCallId()))
                {
                    continue; // OK, will be deleted at end
                }
                if (cseq > contact.getCSeq())
                {
                    continue; // OK
                }
    ...
    private void managePresenceStateChange(String sipAddressOfRecord, PresentityState newState)
    {
        ...
        if (isTraceable(TraceLevel.FINE))
        {
            fine("Firing PresentityStateChangeEvent event from RegistrarSbb with sip address of record of: " + sipAddressOfRecord + "; presence state is: " + newState.toString());
        }
