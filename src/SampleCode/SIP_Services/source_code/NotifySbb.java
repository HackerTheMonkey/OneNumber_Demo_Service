...
public abstract class NotifySbb extends PresenceAwareSbb
{
        ...
        try
        {
            ...
            if (request.getHeader(EventHeader.NAME) != null)
            {
                ...
                if (eventType.equals("presence"))
                {
                    ...
                    {
                        fine("NotifySbb detected presence event for received request");
                    }
                }
                else
                {
                    if (isTraceable(TraceLevel.FINE))
                    {
                        fine("WARNING: NotifySbb detected a non-presence event for received SUBSCRIBE request");
                    }
            ...
            {
                finer("created " + (d.isServer() ? "UAS" : "UAC") + " dialog [id=" + d.getDialogId() + "]");
            }
            ...
            if (isTraceable(TraceLevel.FINEST))
            {
                finest("NotifySbb sending initial NOTIFY request:");
            }
            ...
            if (curPresenceState != null && !curPresenceState.equals(PRESENTITY_OFFLINE))
            {
                sendNewPresenceInfo(d, sipAddressOfRecord, curPresenceState);
            }
    ...
    private void sendErrorResponse(ServerTransaction st, Request request, int errorCode)
    {
        try
        {
            ...
            if (isTraceable(TraceLevel.FINE))
            {
                fine("Sending bad event response: " + response);
            }
        }
    ...
    private String generatePresenceXml(String uri, PresentityState status)
    {
        ...
        xml.append("</basic>");
        xml.append("<note>");
        if (status.getNoteState() != null)
        {
            xml.append(status.getNoteState());
        }
    ...
    public void onNotifyStateChangeEvent(NotifyStateChangeEvent event, ActivityContextInterface aci)
    {
        ...
        if (isTraceable(TraceLevel.FINEST))
        {
            finest("NotifyStateChangeEvent event received by NotifySbb");
        }
    ...
    public void onRefreshTimer(TimerEvent timerEvent, SubscriptionACI aci)
    {
        ...
        try
        {
            ...
            if (isTraceable(TraceLevel.FINEST))
            {
                finest("Subscription timer has expired. Original request for this dialog is:\n" + getCurrentDialog().getFirstTransaction().getRequest());
            }
            if (isTraceable(TraceLevel.FINEST))
            {
                finest("Sending final NOTIFY and tearing down sbb");
            }
    ...
    public void onTransactionTimeOutEvent(TimeoutEvent event, ActivityContextInterface aci)
    {
        ...
        if (isTraceable(TraceLevel.FINEST))
        {
            finest("TransactionTimeOut event received by presence sbb.");
        }
        Request request;
        Transaction tx;
        ...
        if (isTraceable(TraceLevel.FINEST))
        {
            finest("Original request that timed out is:\n" + request);
        }
    ...
    public void onOKResponseEvent(ResponseEvent response, ActivityContextInterface aci)
    {
        ...
        if (isTraceable(TraceLevel.FINEST))
        {
            finest("Response event received by NotifySbb :\n" + response.getResponse());
        }
    }
    ...
    public void onInDialogSubscribeEvent(RequestEvent event, ActivityContextInterface aci)
    {
        try
        {
            if (isTraceable(TraceLevel.FINEST))
            {
                finest("InDialog SUBSCRIBE event received by NotifySbb: \n" + event.getRequest());
            }
    ...
    private boolean isSubscriptionTimeoutCompliant(Request request)
    {
        ...
        try
        {
            if (request.getHeader(UserAgentHeader.NAME) != null)
            {
                ...
                while (iter.hasNext())
                {
                    if (!agentIsCompliant((String) iter.next()))
                    {
                        isCompliant = false;
                    }
                }
        ...
        catch (LazyParsingException e)
        {
            ...
            if (!agentIsCompliant(((LazyParsedMessage) request).getUnparsedHeaderValue(UserAgentHeader.NAME)))
            {
                isCompliant = false;
            }
        }
    ...
    private boolean agentIsCompliant(String userAgentProduct)
    {
        ...
        for (int i = 0; i < expiryTimeNonCompliant.length; i++)
        {
            if (expiryTimeNonCompliant[i].equalsIgnoreCase(userAgentProduct))
            {
                if (isTraceable(TraceLevel.FINEST))
                {
                    finest("Found a non-compliant user-agent: " + userAgentProduct);
                }
                return false;
            }
        }
    ...
    private PresentityState getPresenceState(String uri) throws CreateException
    {
        if (isTraceable(TraceLevel.FINEST))
        {
            finest("Searching for PresentityState via Presence Service using: " + uri + " for search key...");
        }
    ...
    private void tearDown()
    {
        ...
        if (getSubscriptionTimerId() != null)
        {
            getTimerFacility().cancelTimer(getSubscriptionTimerId());
        }
