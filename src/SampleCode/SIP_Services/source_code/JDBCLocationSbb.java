...
public abstract class JDBCLocationSbb extends BaseSbb
{
    ...
    public void onTimerEvent(TimerEvent event, RegistrationExpiryACI regExpiryACI)
    {
        ...
        {
            finer("received expiration timer event for contact: " + publicAddress + "/" + contactAddress + ", flow=" + flowID);
        }
        ...
        if (contact == null)
        {
            if (isTraceable(TraceLevel.FINER))
            {
                finer("contact " + publicAddress + "/" + contactAddress + " not found, nothing to do");
            }
            return;
        }
        ...
        if (origCallId.equals(contact.getCallId()) && origCSeq == contact.getCSeq())
        {
            if (isTraceable(TraceLevel.FINER))
            {
                finer("contact " + publicAddress + "/" + contactAddress + " has expired, removing");
            }
        ...
        else
        {
            if (isTraceable(TraceLevel.FINER))
            {
                finer("contact " + publicAddress + "/" + contactAddress + " has been updated, not removing");
            }
        }
    }
    ...
    public void updateRegistration(Registration reg)
    {
        ...
        {
            finest("updateRegistration: " + reg.getAddressOfRecord());
        }
        try
        {
    ...
    public void removeRegistration(Registration reg)
    {
        if (isTraceable(TraceLevel.FINEST))
        {
            finest("removeRegistration: " + reg.getAddressOfRecord());
        }
        try
        {
    ...
    private void startExpiryTimer(String publicAddress, Registration.Contact contact)
    {
        ...
        if (isTraceable(TraceLevel.FINEST))
        {
            finest("set registration expiry timer for " + contact.getContactURI() + ", expires in " + contact.getExpiryDelta() + "ms");
        }
    }
    ...
    private class RegistrationImpl implements Registration
    {
        ...
        private ArrayList initContacts() throws SQLException
        {
            if (query == null)
            {
                query = conn.prepareStatement(queryGetUser);
            }
            ...
            while (results.next())
            {
                ...
                {
                    tracer.finest("read record for " + addressOfRecord + "/" + contactAddress);
                }
            }
            results.close();
            query.close();
        ...
        private void deleteAllRecords() throws SQLException
        {
            if (deleteAll == null)
            {
                deleteAll = conn.prepareStatement(deleteAllRegistrations);
            }
        ...
        private void deleteRecords() throws SQLException
        {
            ...
            for (Iterator iter = removeContacts.iterator(); iter.hasNext();)
            {
                ...
                if (next instanceof FlowID)
                {
                    ...
                    if (tracer.isTraceable(TraceLevel.FINEST))
                    {
                        tracer.finest("deleting record for " + getAddressOfRecord() + "/" + flowID);
                    }
                ...
                else
                {
                    ...
                    if (tracer.isTraceable(TraceLevel.FINEST))
                    {
                        tracer.finest("deleting record for " + getAddressOfRecord() + "/" + contactURI);
                    }
        ...
        private void deleteContact(String contactURI) throws SQLException
        {
            if (delete == null)
            {
                delete = conn.prepareStatement(deleteRegistration);
            }
        ...
        private void deleteContact(FlowID flowID) throws SQLException
        {
            if (delete == null)
            {
                delete = conn.prepareStatement(deleteRegistrationByFlow);
            }
            delete.setString(1, addressOfRecord);
        ...
        private void updateRecords() throws SQLException
        {
            ...
            for (Iterator iter = updateContacts.iterator(); iter.hasNext();)
            {
                ...
                if (tracer.isTraceable(TraceLevel.FINEST))
                {
                    tracer.finest("updating record for " + getAddressOfRecord() + "/" + contact.getContactURI());
                }
        ...
        private void updateContact(ContactImpl contact) throws SQLException
        {
            if (update == null)
            {
                update = conn.prepareStatement(updateRegistration);
            }
        ...
        private void insertRecords() throws SQLException
        {
            ...
            for (Iterator iter = insertContacts.iterator(); iter.hasNext();)
            {
                ...
                if (tracer.isTraceable(TraceLevel.FINEST))
                {
                    tracer.finest("inserting record for " + getAddressOfRecord() + "/" + contact.getContactURI());
                }
        ...
        private void insertContact(ContactImpl contact) throws SQLException
        {
            if (insert == null)
            {
                insert = conn.prepareStatement(insertRegistration);
            }
        ...
        public Contact removeContact(String contactURI)
        {
            ...
            else
            {
                ...
                if (tracer.isTraceable(TraceLevel.FINEST))
                {
                    tracer.finest("removeContact: " + contactURI);
                }
                ContactImpl contact = (ContactImpl) contacts.remove(idx);
                if (null == removeContacts)
                {
                    removeContacts = new ArrayList(2);
                }
        ...
        public Contact removeContact(FlowID flowID)
        {
            ...
            else
            {
                ...
                if (tracer.isTraceable(TraceLevel.FINEST))
                {
                    tracer.finest("removeContact: flow=" + flowID);
                }
                ContactImpl contact = (ContactImpl) contacts.remove(idx);
                if (null == removeContacts)
                {
        ...
        public Contact updateContact(String contactURI, float q, long expires, String callId, int cseq, FlowID flowID)
        {
            ...
            else
            {
                ...
                if (tracer.isTraceable(TraceLevel.FINEST))
                {
                    tracer.finest("updateContact: " + contactURI);
                }
                contacts.remove(idx);
                ...
                if (null == updateContacts)
                {
                    updateContacts = new ArrayList(2);
                }
        ...
        public Contact addContact(String contactURI, float q, long expires, String callId, int cseq, FlowID flowID)
        {
            ...
            if (null == insertContacts)
            {
                insertContacts = new ArrayList(2);
            }
            insertContacts.add(newContact);
            if (tracer.isTraceable(TraceLevel.FINEST))
            {
                tracer.finest("addContact: " + contactURI);
            }
            dirty = true;
            return newContact;
        }
        ...
        private int findIndex(String uri)
        {
            for (int i = 0; i < contacts.size(); i++)
            {
                ...
                if (c.getContactURI().equalsIgnoreCase(uri))
                {
                    return i;
                }
            }
